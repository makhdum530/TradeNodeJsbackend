...
export const fvPassword = (req, key) => {
	...
	if (!passwordRegex.test(value)) {
		throw new Error(`${key} must be at least 8 characters long and include at least one uppercase letter, one number, and one special character.`);
	}
	return value;
};
...
export const fvString = (req, key) => {
	const value = req.body[key] || req.query[key] || req.headers[key] || req.params[key];
