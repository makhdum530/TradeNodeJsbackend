...
export const validateLoginMiddleware = async (req, res, next) => {
	try {
		...
		if (!user) return handleError({ res, message: 'Invalid or inactive user.' });

		req.login_user_id = user_id;
		req.role_id = Number(user?.role_id);
		req.login_user_email = user?.email;

		// Determine the token column based on the device type
		const tokenColumn = devicetype === 'ANDROID' ? 'anroid_token' : devicetype === 'IOS' ? 'ios_token' : 'web_token';
